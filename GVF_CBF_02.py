...
for i in range(1,fin,intervalo):
    ...
    for j in range(N):
        ax.scatter(pi[j,0],pi[j,1],linewidth = 2.5, zorder = 3)

        #campo vectorial - gradiente 
        plt.quiver(pi[j,0],pi[j,1],Chi2[j,0], Chi2[j,1], length=1, color = "olive", zorder = 1)
        plt.quiver(pi[j,0],pi[j,1],Chi2_hat[j,0], Chi2_hat[j,1], length=1, color = "red", zorder = 1)
...
line3, = ax.plot([], [], 'c-', label='Trayectoria3')
point3, = ax.plot([], [], 'co', label='Posición actual3')
